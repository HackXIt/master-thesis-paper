...
%---------------------------------------------------------------Abstract
\outline{%
This thesis investigates the application of \ac{DRL} agents for automated playtesting and game balancing in the action combat dungeon crawler \emph{Shattered Bonds}, developed in \ac{UE5}. 
...
\ac{MLOps} practices will be explored for hyperparameter optimization of the reward function design.
Expected outcomes include a reproducible framework for \ac{RL}-based playtesting in \ac{UE5}, quantitative metrics for difficulty assessment, and empirical comparison between agent and human playtesting data.
}
...
\aitoolentry{Claude (Anthropic)}{Writing assistance and text drafting}{Used for drafting and refining proposal sections including abstract, motivation, and methodology descriptions. All content reviewed and edited by author.}
\aitoolentry{Claude (Anthropic)}{Literature review structuring}{Assisted in organizing and summarizing key findings from academic sources. Citation accuracy verified manually.}
\aitoolentry{Claude (Anthropic)}{LaTeX formatting and document structure}{Used for LaTeX syntax assistance, table formatting, and TikZ figure generation. Code reviewed and adapted by author.}
\aitoolentry{Claude (Anthropic)}{Proofreading and revision}{Used for grammar checking, consistency review, and citation format verification throughout the document.}
...
The game industry has recognized automated playtesting as a potential solution \cite{gordilloImprovingPlaytestingCoverage2021, ferdousAgentBasedTesting3D2022}. \ac{DRL} has shown promise in learning complex sequential decision-making tasks, including playing video games at superhuman levels \cite{vinyalsGrandmasterLevelStarCraft2019, openaiDota2Large2019}. However, applying \ac{DRL} to action combat games for \emph{balancing} rather than merely \emph{winning} presents unique challenges.
...
\begin{itemize}
  \item \textbf{Deep Reinforcement Learning}: Applying state-of-the-art algorithms (\ac{PPO}, \ac{SAC}) to complex continuous control tasks
  \item \textbf{Curriculum Learning}: Structuring training to progressively increase task difficulty \cite{bengioCurriculumLearning2009}
  \item \textbf{Reward Engineering}: Designing reward functions that produce desired agent behaviors \cite{ngPolicyInvarianceReward1999}
  ...
  \item \textbf{\ac{MLOps} for \ac{RL}}: Exploring systematic approaches to hyperparameter optimization for reinforcement learning systems \cite{zahariaAcceleratingMachineLearning, RstudioMlfloworiginal2023}
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Real-time 3D combat}: Most \ac{RL} playtesting research focuses on simpler 2D games or turn-based systems. Action combat in 3D presents significantly higher complexity in state and action spaces.
  ...
  \item \textbf{\ac{UE5} integration}: Establishing a reproducible framework for \ac{RL} training within \ac{UE5}, building on recent developments like Learning Agents \cite{AlanLaboratoryUnrealMLAgents2025}.
...
The application of reinforcement learning to game testing has gained significant attention in recent years. Gordillo et al. demonstrated that deep reinforcement learning can augment automated game testing by learning to explore game mechanics and detect bugs without explicit scripting \cite{gordilloImprovingPlaytestingCoverage2021}. Their work on curiosity-driven exploration showed that \ac{RL} agents can achieve meaningful test coverage in complex 3D environments.

Ferdous et al. \cite{ferdousAgentBasedTesting3D2022} presented an approach for agent-based testing of 3D games using curiosity as a motivating factor. Their results provide preliminary evidence that \ac{RL} can be adopted for automated testing, though they note challenges with complex game mechanics.

The Wuji framework represents a significant advancement, combining evolutionary algorithms with deep reinforcement learning for combat game testing \cite{zhengWujiAutomaticOnline2019}. Zheng et al. demonstrated that this hybrid approach could discover bugs in commercial games by balancing between task completion and state space exploration. Their multi-objective optimization approach is particularly relevant to our work.
...
Noblega et al. \cite{noblegaAdaptiveDeepReinforcement2019} explored adaptive deep reinforcement game balancing, proposing reward functions based on balancing constants. Their work demonstrates that \ac{RL} agents can maintain balance while adapting to different player skill levels.
...
Curriculum learning \cite{bengioCurriculumLearning2009} structures training by progressively increasing task difficulty. In \ac{RL} contexts, this has proven effective for complex tasks where naive training fails to converge. OpenAI's work on Dota 2 \cite{openaiDota2Large2019} and DeepMind's AlphaStar \cite{vinyalsGrandmasterLevelStarCraft2019} both employed curriculum strategies to achieve superhuman performance in complex games.
...
Buşoniu et al. \cite{busoniuMultiagentReinforcementLearning2010} provide a comprehensive overview of \ac{MARL}, discussing cooperative, competitive, and mixed scenarios. Tan's foundational work \cite{tanMultiAgentReinforcementLearning1993} on independent versus cooperative agents established key principles for multi-agent systems.

Papoudakis et al. \cite{papoudakisDealingNonStationarityMultiAgent2019} address the non-stationarity problem in multi-agent deep \ac{RL}, surveying methods from centralized training to meta-learning. These insights are relevant if we pursue a multi-agent architecture where different agents specialize in different combat aspects.
...
\begin{enumerate}
  ...
  \item \textbf{Balance-focused rather than win-focused}: Existing \ac{RL} game agents optimize for winning; our work requires agents that produce useful balance feedback.

  \item \textbf{\ac{UE5} integration}: Limited academic work exists on \ac{RL} integration with \ac{UE5} for production games.

  \item \textbf{Comparative validation}: Few studies compare \ac{RL}-generated telemetry directly with human playtester data using statistical methods.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Agent Architecture}: Designing the \ac{RL} agent(s) for combat control
...
\begin{itemize}
  \item \textbf{\ac{UE5} Learning Agents}: Epic's official ML plugin for reinforcement and imitation learning
...
\begin{itemize}
  ...
  \item Algorithm: \ac{PPO} or \ac{SAC}
...
\subsection{\ac{MLOps} Integration}

Experiment tracking and hyperparameter optimization:

\begin{itemize}
  \item \textbf{MLflow} \cite{zahariaAcceleratingMachineLearning}: Track training runs, log metrics, version models
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}clp{6cm}c@{}}
    ...
    \midrule
    1           & Setup          & \ac{UE5}-Python communication established, basic agent training loop functional & Month 1         \\
    \addlinespace
    ...
    \addlinespace
    7           & Optimization   & Reward shaping optimized, \ac{MLOps} pipeline functional                        & Month 4         \\
    \addlinespace
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      % Dummy data showing typical \ac{RL} training curve
...
\begin{enumerate}
  \item \textbf{Trained \ac{RL} Agent}: An agent capable of completing combat encounters in \emph{Shattered Bonds} with measurable success rates

  \item \textbf{\ac{UE5} Training Framework}: Documented codebase for \ac{RL} training in \ac{UE5}, potentially reusable for similar projects
...
\begin{enumerate}
  ...
  \item Statistical framework for validating \ac{RL} playtesting against human baselines

  \item Practical guidelines for integrating \ac{RL} playtesting into game production
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{\ac{UE5} integration complexity}: Technical challenges with engine integration may require fallback approaches
\end{itemize}
